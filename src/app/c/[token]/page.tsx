...
import { readChecklist, type ChecklistItem } from "@/lib/checklist";
...
const CIRC = 2 * Math.PI * 32; // anel de progresso (r=32)

/* Visual premium, escuro e cinematográfico (tema único — a experiência do cliente é uma "sala" de marca). */
const STYLE = `
.cp{
...
/* FILEIRA DE PÔSTERES (materiais do projeto) */
...
document.querySelectorAll('.cp .chip').forEach(function(c){
  c.addEventListener('click',function(){
    document.querySelectorAll('.cp .chip').forEach(function(x){x.classList.remove('on')});
    c.classList.add('on');
    var f=c.getAttribute('data-f');
    document.querySelectorAll('.cp .poster').forEach(function(m){
...
export default async function ClientePage({ params }: { params: Promise<{ token: string }> }) {
  ...
  const project = await prisma.setorClickupList.findUnique({
    ...
    select: {
      ...
      internalTasks: {
        ...
        select: { id: true, title: true, description: true, stage: true, checklist: true, done: true, dueDate: true },
      },
      materials: {
  ...
  if (!project) {
    return (
      <div className="cp">
        ...
        <div className="cpw"><p className="empty">Link inválido ou expirado.</p></div>
      </div>
    );
  }

  const now = new Date();
  const tasks: Task[] = project.internalTasks.map((t) => ({
    id: t.id, title: t.title, description: t.description, stage: t.stage,
    checklist: readChecklist(t.checklist), done: t.done, dueDate: t.dueDate,
  }));
  const doneCount = tasks.filter((t) => t.done).length;
  const pct = tasks.length ? Math.round((doneCount / tasks.length) * 100) : 0;

  // Materiais por tarefa + soltos (nível projeto).
  const matsByTask = new Map<string, Mat[]>();
  const looseMats: Mat[] = [];
  for (const m of project.materials as Mat[]) {
    if (m.taskId) {
  ...
  // Agrupa tarefas por etapa (preserva ordem de criação).
      ...
      <div className={`tk ${t.done ? "done" : ""}`}>
        <div className="tkh">
          ...
          <div className="tkmain">
            ...
            {mats.length > 0 && (
              <div className="tkmats">
                {mats.map((m) => <TaskMat key={m.id} m={m} />)}
              </div>
            )}
          </div>
        </div>
    ...
    <div className="cp">
      ...
      <div className="cpw">
        <header className="hero">
          <div className="hmain">
            <div className="hbadge"><span className="dt" />{project.clientCompany?.name ?? "Cliente"}</div>
            <h1>{project.name}</h1>
        ...
        <footer className="foot">Atendido por <b>Azz</b> · Diego Lazzari</footer>
      </div>
