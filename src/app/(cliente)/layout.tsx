...
export default async function ClienteLayout({ children }: { children: React.ReactNode }) {
  ...
  const company = await prisma.company.findUnique({
    where: { id: companyId },
    select: { name: true, parentCompanyId: true, parentCompany: { select: { name: true } } },
  });
  ...
  return (
    <>
      <IconGradients />
      <ClientShell clientName={company.name} agencyName={company.parentCompany?.name ?? "Portal"} banner={banner}>
        {children}
      </ClientShell>
    </>
  );
