...
export default function InstagramManager() {
    ...
    return (
      <div className="p-6 max-w-2xl">
        ...
        <div className="mt-6 rounded-xl border border-white/10 bg-white/5 p-6">
          ...
          <a
            href="/configuracoes?secao=integracoes-meta"
          ...
          >
            <Camera className="w-4 h-4" /> Conectar em Configurações → Meta
          </a>
        </div>
      </div>
    );
  ...
  return (
    <div className="p-6 max-w-4xl">
      ...
      {/* Conexão mora em Configurações → Integrações → Meta. Aqui fica só o
          estado atual e o atalho — conectar é configuração, não operação. */}
      <div className="mt-4 flex items-center gap-3 rounded-xl border border-white/10 bg-white/5 p-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-[11px] text-emerald-400">
            Conectado
            {fbPages.length > 0 && ` · ${fbPages.length} página${fbPages.length > 1 ? "s" : ""} do Facebook`}
          </p>
        </div>
        <a
          href="/configuracoes?secao=integracoes-meta"
          className="rounded-lg border border-white/10 px-3 py-1.5 text-xs text-slate-300 hover:bg-white/5 flex-shrink-0"
        >
          Configurar conexão →
        </a>
      </div>
