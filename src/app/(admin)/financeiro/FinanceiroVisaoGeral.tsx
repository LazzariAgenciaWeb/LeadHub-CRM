      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className={card}>
          ...
          {data.ativos.length === 0 ? (
            ...
            /* Agrupada por serviço, com o total da área no cabeçalho do grupo —
               "o que tenho de cada serviço e quanto rende" sem sair da lista. */
            <div className="space-y-3 max-h-[420px] overflow-y-auto">
              {porServico
                .filter((s) => ativosVisiveis.some((a) => a.tipo === s.tipo))
                .map((s) => {
                  const doGrupo = ativosVisiveis.filter((a) => a.tipo === s.tipo);
                  const subtotalMensal = doGrupo.reduce((n, a) => n + a.mensalCents, 0);
                  return (
                    <div key={s.tipo}>
                      <div className="flex items-baseline justify-between gap-3 px-1 pb-1 border-b border-[#1e2d45] mb-1.5 sticky top-0 bg-[#0f1623]">
                        <span className="text-xs font-semibold text-slate-300 truncate">{s.tipo}</span>
                        <span className="text-[11px] text-slate-500 flex-shrink-0">
                          {doGrupo.length} contrato(s) · <b className="text-indigo-300">{brlFromCents(subtotalMensal)}/mês</b>
                        </span>
                      </div>
